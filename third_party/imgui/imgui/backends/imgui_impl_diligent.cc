 ...
 *  Copyright 2018-2025 Admenri Adev
 *
...
void ImGuiDiligentRenderer::UpdateTexture(IDeviceContext* pCtx,
                                          ImTextureData* pTexData) {
  ...
  if (pTexData->Status == ImTextureStatus_WantCreate) {
    ...
    ITextureView* ptexView =
        pTexture->GetDefaultView(TEXTURE_VIEW_SHADER_RESOURCE);
    VERIFY_EXPR(ptexView != nullptr);

    // store texture view and texture pointers inside imgui and set texture
    // state to ok
  ...
  } else if (pTexData->Status == ImTextureStatus_WantUpdates &&
             pTexture != nullptr) {
    Box dstBox{
    ...
    SubresData.pData =
        pTexData->GetPixelsAt(pTexData->UpdateRect.x, pTexData->UpdateRect.y);
    SubresData.Stride = pTexData->GetPitch();

    pCtx->UpdateTexture(pTexture, 0, 0, dstBox, SubresData,
                        RESOURCE_STATE_TRANSITION_MODE_VERIFY,
                        RESOURCE_STATE_TRANSITION_MODE_TRANSITION);
    pTexData->SetStatus(ImTextureStatus_OK);
  } else if (pTexData->Status == ImTextureStatus_WantDestroy &&
             pTexData->UnusedFrames > 0) {
    ...
    for (const ImDrawCmd& Cmd : pCmdList->CmdBuffer) {
      ...
      if (pCmd->UserCallback != nullptr) {
        ...
        ITextureView* pTextureView =
            reinterpret_cast<ITextureView*>(pCmd->GetTexID());
